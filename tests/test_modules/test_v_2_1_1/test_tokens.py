...
def test_cpo_get_token_v_2_1_1():
    app = get_application(
    ...
    response = client.get(
        f"/ocpi/cpo/2.1.1/tokens/{settings.COUNTRY_CODE}/{settings.PARTY_ID}/"
        f'{TOKENS[0]["uid"]}'
    )
...
def test_cpo_add_token_v_2_1_1():
    app = get_application(
        version_numbers=[VersionNumber.v_2_1_1],
        roles=[enums.RoleEnum.cpo],
        crud=Crud,
...
def test_cpo_update_token_v_2_1_1():
    app = get_application(
        version_numbers=[VersionNumber.v_2_1_1],
        roles=[enums.RoleEnum.cpo],
        crud=Crud,
...
def test_emsp_get_tokens_v_2_1_1():
    app = get_application(
        version_numbers=[VersionNumber.v_2_1_1],
    ...
    client = TestClient(app)
    response = client.get("/ocpi/emsp/2.1.1/tokens")
...
def test_emsp_authorize_token_success_v_2_1_1():
    app = get_application(
        version_numbers=[VersionNumber.v_2_1_1],
        roles=[enums.RoleEnum.emsp],
        crud=Crud,
    ...
    response = client.post(
        f'/ocpi/emsp/2.1.1/tokens/{TOKENS[0]["uid"]}/authorize'
    )
...
def test_emsp_authorize_token_unknown_v_2_1_1():
    @classmethod
    async def get(
        cls,
        module: enums.ModuleID,
...
def test_emsp_authorize_token_missing_info_v_2_1_1():
    @classmethod
    async def do(
        cls,
        module: enums.ModuleID,
