...
def test_cpo_get_location_v_2_1_1():
    ...
    client = TestClient(app)
    response = client.get(f'/ocpi/cpo/2.1.1/locations/{LOCATIONS[0]["id"]}')
...
def test_cpo_get_evse_v_2_1_1():
    ...
    client = TestClient(app)
    response = client.get(
        f'/ocpi/cpo/2.1.1/locations/{LOCATIONS[0]["id"]}/{LOCATIONS[0]["evses"][0]["uid"]}'
    )
...
def test_emsp_get_location_v_2_1_1():
    ...
    client = TestClient(app)
    response = client.get(
        f"/ocpi/emsp/2.1.1/locations/{settings.COUNTRY_CODE}/{settings.PARTY_ID}"
        f'/{LOCATIONS[0]["id"]}'
    )
